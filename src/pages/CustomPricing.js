...
const CustomPricing = () => {
    ...
    const fetchCustomPricing = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const response = await axios.get('https://api.w1chat.com/admin/subscription/all-custom-pricing', {
                headers: { 'x-token': tokens.token }
            });

            if (!response.data.error) {
                setCustomPricing(response.data.data || []);
            }
        } catch (err) {
            console.error("Failed to fetch custom pricing", err);
            setError('Failed to fetch custom pricing data');
        } finally {
            setLoading(false);
        }
    }, [tokens]);
    ...
    }, [fetchCustomPricing, tokens]);

    const handleRemovePricing = async (username, packId) => {
        ...
        try {
            ...
            setError(err?.response?.data?.message || 'Failed to remove pricing');
        }
    };

    const filteredPricing = customPricing.filter(item => {
        const matchesSearch = 
            item.username?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            item.user_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            item.email?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            item.pack_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            item.pack_id?.toLowerCase().includes(searchTerm.toLowerCase());

        const matchesFilter = 
            filterType === 'all' ||
            (filterType === 'active' && item.is_active === '1') ||
            (filterType === 'inactive' && item.is_active === '0') ||
            (filterType === 'platform' && item.pack_type === 'platform') ||
            (filterType === 'addon' && item.pack_type === 'addon');
    ...
    const stats = {
        ...
        active: customPricing.filter(p => p.is_active === '1').length,
        users: new Set(customPricing.map(p => p.username)).size,
        totalSavings: customPricing.reduce((sum, p) => {
            const defaultAmt = parseFloat(p.default_amount || 0);
            const customAmt = parseFloat(p.custom_amount || 0);
            return sum + (defaultAmt - customAmt);
        }, 0)
    };
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-8">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-400">Manage user-specific pricing across all packs</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-sm text-gray-500 dark:text-gray-400">Total Custom Pricing</p>
                        ...
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-sm text-gray-500 dark:text-gray-400">Active Pricing</p>
                        ...
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <h3 className="text-2xl font-bold dark:text-white">{formatCurrency(stats.totalSavings)}</h3>
                                </div>
                                        ...
                                        <>
                                            ...
                                            <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-lg z-50 py-1">
                                                {[
                                                    ...
                                                    { value: 'platform', label: 'Platform Packs' },
                                                    { value: 'addon', label: 'Add-on Packs' }
                                                ].map(filter => (
                                                    <button
                                                        key={filter.value}
                                                        onClick={() => {
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                        {loading ? (
                            ...
                            <div className="text-center py-20">
                                <FiDollarSign className="mx-auto text-gray-300 dark:text-gray-700 mb-3" size={48} />
                                <p className="text-gray-500 dark:text-gray-400 font-medium">No custom pricing found</p>
                            </div>
                        ) : (
                                ...
                                <table className="w-full text-left">
                                    <thead className="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-700">
                                        <tr>
                                            ...
                                            <th className="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Default Price</th>
                                            <th className="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Custom Price</th>
                                    ...
                                    <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                        {filteredPricing.map((item) => {
                                            const defaultAmt = parseFloat(item.default_amount || 0);
                                            const customAmt = parseFloat(item.custom_amount || 0);
                                            const discount = defaultAmt - customAmt;
                                            const discountPercent = defaultAmt > 0 ? ((discount / defaultAmt) * 100).toFixed(1) : 0;

                                            return (
                                                <tr key={item.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
                                                    <td className="px-6 py-4">
                                                        <div>
                                                            <div className="font-medium text-gray-900 dark:text-white">{item.user_name || item.username}</div>
                                                            <div className="text-sm text-gray-500 dark:text-gray-400">{item.email}</div>
                                                            <div className="text-xs text-gray-400 dark:text-gray-500 font-mono">{item.username}</div>
                                                        </div>
                                                    </td>
                                                    <td className="px-6 py-4">
                                                        <div>
                                                            <div className="font-medium text-gray-900 dark:text-white">{item.pack_name}</div>
                                                            <div className="flex items-center gap-2 mt-1">
                                                                <span className={`px-2 py-0.5 rounded text-[10px] font-semibold uppercase ${
                                                                    item.pack_type === 'platform'
                                                                        ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400'
                                                                        : 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400'
                                                                }`}>
                                                                    {item.pack_type}
                                                                </span>
                                                                <span className="text-xs text-gray-400 font-mono">{item.pack_id}</span>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="px-6 py-4">
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <span className="font-semibold text-gray-900 dark:text-white">
                                                            {formatCurrency(item.custom_amount)}
                                                        </span>
                                                    </td>
                                                    <td className="px-6 py-4">
                                                    ...
                                                    <td className="px-6 py-4 text-right">
                                                        <button
                                                            onClick={() => handleRemovePricing(item.username, item.pack_id)}
